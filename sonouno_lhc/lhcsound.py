    ...
    for track in track_list:
        ...
        if not track_elements[0] in sonified_tracks_list:
            ...
            if count < len(track_list):
                for track2 in track_list_2[count:]:
                    ...
                    value_tracks = math.sqrt(
                        pow((float(track_elements[4]) - float(track2_elements[4])), 2)
                        + pow((float(track_elements[5]) - float(track2_elements[5])), 2)
                    )
            ...
            if cluster_tosonify:
                ...
                if converted_photon == ' ':
                    print(
                        'Sonifying ' + track_elements[0] + ' and ' + cluster_elements[0]
                    )
...
double_track_sound = get_sine_wave(freqD6, duration=2) + get_sine_wave(
    freqC6, duration=2
)
